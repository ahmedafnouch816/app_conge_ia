        ...
        try:
            ...
            if serializer.is_valid():
                ...
                if prediction is None:
                    return Response({"error": statut_recommande}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({'status': 400, 'message': 'Données invalides.', 'errors': serializer.errors}, status=status.HTTP_400_BAD_REQUEST)

        except Employe.DoesNotExist:
            return Response({'status': 403, 'message': 'Employé non autorisé.'}, status=status.HTTP_403_FORBIDDEN)



# end pont de recuperes data de recommendations
class GetRecommandationView(APIView):
    authentication_classes = [TokenAuthentication]  # ✅ Utilisation de TokenAuthentication
    #permission_classes = [IsAuthenticated]  # 🔒 Seuls les utilisateurs authentifiés peuvent accéder

    def get(self, request, demande_id):
        try:
            recommandation = Recommandation.objects.select_related('demande').get(demande_id=demande_id)
            return Response({
                'demande': {
            ...
            return Response({'error': 'Aucune recommandation trouvée pour cette demande.'}, status=status.HTTP_404_NOT_FOUND)
